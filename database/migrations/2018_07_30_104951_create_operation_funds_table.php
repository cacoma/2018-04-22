...
class CreateOperationFundsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('operation_funds', function (Blueprint $table) {
            $table->increments('id');
      ...
      Schema::table('operation_funds', function($table) {
            //foreign keys
            $table->foreign('invest_id')->references('id')->on('invests');
            $table->foreign('fund_id')->references('id')->on('funds');
            $table->foreign('user_id')->references('id')->on('users');
            $table->foreign('broker_id')->references('id')->on('brokers');         
            DB::statement('ALTER TABLE operation_funds ADD total_operation DECIMAL(18,2) AS (price_operation * quant_operation - broker_fee_operation)');
            ...
            DB::statement('ALTER TABLE operation_funds ADD duration_days int(18) AS (TIMESTAMPDIFF(day, date_invest,date_operation))');
        });
    }
    ...
    {
        Schema::dropIfExists('operation_funds');
    }
}
